	...
	void addToCache(int wordAddress) {
		...
		if (cache[index / setSize].Lines[index % setSize].ValidBit) {
			...
			if (this.WriteBack && DirtyBits[index]) {
				String memAddress = cache[index / setSize].Lines[index
						% setSize].Tag
						+ Integer.toBinaryString(index);

				// adding zeroes to adjust for missing offset bits in extracted
				// address
				// this gives us the address of the start of block for this
				// specific cache
				for (int i = 0; i < this.lengthOffset; i++)
					memAddress += "0";
				MainMemory.Insert(memAddress,
						cache[index / setSize].Lines[index % setSize].Data,
						this.BlockSize);
			}
		}
		// cycle Calc $$$$$$$$$$$$$$$
	...
	public String Read(int wordAddress) {
		...
		for (int i = 0; i < setSize; i++) {
			...
			if (cache[index / setSize].Lines[i].ValidBit) {
				...
				// The block in the index is not the one we are looking for so
				// it must be replaced
				// We can return null too since the read already adds the
				// correct blocks from memory
	...
	public boolean Write(int wordAddress, String data) {
		...
		for (int i = 0; i < DirtyBits.length; i++) {
			...
			if (cache[index / setSize].Lines[i].ValidBit) {
				if (cache[index / setSize].Lines[i].Tag.equals(tagBinary)) {
					if (this.WriteBack) {
						this.DirtyBits[(index - (index % setSize)) + i] = true;
					}
